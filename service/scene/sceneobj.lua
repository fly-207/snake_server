...
local effectobj = import(service_path("effectobj"))
...
function CScene:EnterGMPlayer(iPid, iEid, mPos, mInfo, iSpeed)
    assert(not self.m_mGMPlayers[iPid], string.format("EnterGMPlayer error %d %d", iPid, iEid))
    local obj = playerobj.NewPlayerEntity(iEid, iPid)
    self.m_mGMPlayers[iPid] = iEid
    obj:Init({
        scene_id = self:GetSceneId(),
        pos = mPos,
        speed = iSpeed,
...
function CScene:LeaveGMPlayer(iPid)
    local eid = self.m_mGMPlayers[iPid]
    local obj = self:GetEntity(eid)
    if obj then
        self:Leave(obj)
        self.m_mGMPlayers[iPid] = nil
    else
        record.warning("LeavePlayer err: %s %s no entity", iPid, eid)
    end
end
...
function CScene:EnterEffect(iEid, iEffId, mPos, mInfo)
    local obj = effectobj.NewEffectEntity(iEid, iEffId)
    obj:Init({
        scene_id = self:GetSceneId(),
        pos = mPos,
        speed = 0,
...
function CScene:RemoveSceneEffect(iEid)
    local oEntity = self:GetEntity(iEid)
    if oEntity then
        assert(oEntity:IsEffect(), string.format("RemoveSceneEffect err :%d",iEid))
        self:Leave(oEntity)
    end
end

function CScene:CreateSceneTeam(iEid, iTeam, mMem, mShort)
    ...
    local obj = teamobj.NewTeamEntity(iEid, iTeam, mMem, mShort)
...
function CScene:UpdateSceneTeam(iTeam,mMem,mShort)
...
function CScene:BroadcastMessage(sMessage, mData, mExclude)
    ...
    for iPid, iEid in pairs(self.m_mPlayers) do
        if mExclude[iPid] then goto continue end

        local oEntity = self:GetPlayerEntity(iPid)
        if not oEntity then goto continue end

        oEntity:SendRaw(sData)
        ::continue::
    end
end
